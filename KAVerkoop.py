...
column_query = "SELECT COLUMN_NAME, DATA_TYPE FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME = 'Verkoop'"
cursor.execute(column_query)
columns = cursor.fetchall()

null_count_queries = [f"SELECT COUNT(*) FROM Verkoop WHERE {column} IS NULL" for column, _ in columns]
...
for i, (column, data_type) in enumerate(columns):
    ...
    if is_numeric:
        number_count_query = f"SELECT COUNT(*) FROM Verkoop WHERE ISNUMERIC({column}) = 1 AND {column} IS NOT NULL"
        cursor.execute(number_count_query)
        number_count = cursor.fetchone()[0]
    else:
        number_count = 0

    letter_count_query = f"SELECT COUNT(*) FROM Verkoop WHERE {column} LIKE '%[A-Za-z]%' AND {column} IS NOT NULL"
